# Character Creator V4 — Task 列表 (第三轮)

> **创建时间**: 2026-04-07 (第三轮)
> **目标**: 从 V1（Phaser 为主）翻转为 V4（Vue 为主 + Phaser 仅 CanvasPreview）
> **约束**: core/ 不修改；所有接口必须对照 V1 源码 + App.vue 调用方
> **代码量表**: `CC_V4_RUBRIC.md` | **需求量表**: `CC_V4_REQUIREMENTS_RUBRIC.md`

---

## V1 → V4 组件对照（仅保留 V1 中确实存在的）
...
| # | Task | 文件 | 说明 |
|---|------|------|------|
| T01 | 目录结构 + types.ts | `v4/`, `types.ts` | 创建目录，定义 CreatorStep 类型 |
