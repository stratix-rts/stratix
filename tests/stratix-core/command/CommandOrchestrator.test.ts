...
function makeMockExecutor(overrides: {
  ...
  executeImpl?: () => Promise<ExecutorResult>;
  validateImpl?: () => { valid: boolean; errors: string[] };
  testConnectionImpl?: () => Promise<{ success: boolean; message: string }>;
} = {}): jest.Mocked<AgentExecutor> {
  const executeImpl = overrides.executeImpl ?? (() =>
    Promise.resolve(overrides.executeResult ?? { success: true, data: 'ok' })
  );
  const validateImpl = overrides.validateImpl ?? (() =>
    overrides.validateResult ?? { valid: true, errors: [] }
  );
  const testConnectionImpl = overrides.testConnectionImpl ?? (() =>
    Promise.resolve(overrides.testConnectionResult ?? { success: true, message: 'ok' })
  );

  const mock: Partial<AgentExecutor> = {
    execute: jest.fn().mockImplementation(executeImpl),
    validate: jest.fn().mockImplementation(validateImpl),
    testConnection: jest.fn().mockImplementation(testConnectionImpl),
  };

  return mock as jest.Mocked<AgentExecutor>;
}
...
describe('CommandOrchestrator', () => {
  ...
  describe('executeWithResult', () => {
    ...
    it('propagates errors thrown by executor', async () => {
      const { spy } = withMockFactory(
        makeMockExecutor({
          executeImpl: () => Promise.reject(new Error('execute error')),
        })
      );
    ...
    it('passes correct command and config to executor', async () => {
      ...
      expect(mockExec.execute).toHaveBeenCalledWith(cmd, config);
      spy.mockRestore();
    });
  });
  ...
  describe('validateCommand', () => {
    ...
    it('returns valid: false with errors when validation fails', () => {
      const { spy } = withMockFactory(
        makeMockExecutor({
          validateResult: { valid: false, errors: ['Provider is required', 'Model is required'] },
        })
      );
    ...
    it('propagates errors thrown by executor validate', () => {
      const { spy } = withMockFactory(
        makeMockExecutor({
          validateImpl: () => {
  ...
  describe('testConnection', () => {
    ...
    it('propagates errors thrown by executor testConnection', async () => {
      const { spy } = withMockFactory(
        makeMockExecutor({
          testConnectionImpl: () => Promise.reject(new Error('testConnection error')),
        })
      );
