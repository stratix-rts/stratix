...
/**
 * CanvasPreview.vue — Phaser Canvas 预览组件 (V4)
 *
 * 唯一接触 Phaser 的组件。
...
// 纹理 key
let sheetTextureKey = 'char_preview_sheet';
let frameTextureKey = 'char_preview_frame';
...
// ============================================================================
// 棋盘格背景（来源：V1 CharacterCreatorScene createCheckerboardTexture）
// ============================================================================

function createCheckerboardTexture(sc: Phaser.Scene, width: number, height: number): string {
  ...
  sc.textures.addCanvas(key, canvas);
  return key;
}
...
async function renderCharacter(): Promise<void> {
  ...
  try {
    ...
    // Recalculate frame indexes for current animation + direction
    updateFrameIndexes();
    displayCurrentFrame();
  } catch (error) {
...
function displayCurrentFrame(): void {
  ...
  // Draw the frame from spritesheet
  ctx.drawImage(
  ...
  // Ensure checkerboard background exists
