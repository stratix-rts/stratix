...
/**
 * BackendSelector.vue - 后端选择器 (V3)
 *
 * 支持 OpenClaw / StratixAgent / Direct 三种后端
 * OpenClaw 配置：endpoint, accountId, apiKey
 * Stratix 配置：provider, model, apiKey, endpoint, temperature, maxTokens
 * Direct 配置：provider, model, apiKey, endpoint
 *
 * Props:
...
export type AgentBackendType = 'openclaw' | 'stratix' | 'direct';
...
const stratixModelOptions = computed(() => modelOptions[stratixConfig.value.provider] ?? modelOptions.custom);
const directModelOptions = computed(() => modelOptions[directConfig.value.provider] ?? modelOptions.custom);
...
function emitChange(): void {
  const value = {
    ...
    openClawConfig: currentBackend.value === 'openclaw' ? { ...openClawConfig.value } : undefined,
    stratixConfig: currentBackend.value === 'stratix' ? { ...stratixConfig.value } : undefined,
    directConfig: currentBackend.value === 'direct' ? { ...directConfig.value } : undefined,
  };
  emit('update:modelValue', value);
  emit('change', value);
}

watch([openClawConfig, stratixConfig, directConfig], () => {
  emitChange();
}, { deep: true });
...
<template>
  <div class="backend-selector">
    ...
    <div class="backend-selector__type">
      ...
      <button
        class="backend-type-btn"
        :class="{ 'backend-type-btn--active': currentBackend === 'direct' }"
        @click="selectBackend('direct')"
      >
        <span class="backend-type-btn__icon">🔗</span>
        <span class="backend-type-btn__label">Direct</span>
      </button>
    </div>
    ...
    <div class="backend-selector__config">
      ...
      <div v-show="currentBackend === 'stratix'" class="config-panel">
        ...
        <div class="form-field">
          ...
          <select v-model="stratixConfig.model" class="form-select">
            <option v-for="m in stratixModelOptions" :key="m.value" :value="m.value">
              {{ m.label }}
            </option>
          </select>
        </div>
        <div class="form-field">
          <label class="form-label">
            API Key
            <span v-if="stratixConfig.provider === 'ollama'" class="form-label__optional">(可选)</span>
          </label>
          <input
      ...
      <div v-show="currentBackend === 'direct'" class="config-panel">
        <div class="form-field">
          <label class="form-label">Provider</label>
          <select v-model="directConfig.provider" class="form-select">
            <option v-for="p in providerOptions" :key="p.value" :value="p.value">
              {{ p.label }}
            </option>
          </select>
        ...
        <div class="form-field">
          <label class="form-label">Model</label>
          <select v-model="directConfig.model" class="form-select">
            <option v-for="m in directModelOptions" :key="m.value" :value="m.value">
              {{ m.label }}
            </option>
          </select>
        </div>
        <div class="form-field">
          <label class="form-label">
            API Key
            <span v-if="directConfig.provider === 'ollama'" class="form-label__optional">(可选)</span>
          </label>
          <input
        ...
        <div v-if="directConfig.provider === 'custom'" class="form-field">
          <label class="form-label">Custom Endpoint</label>
          <input
        ...
        <div class="form-row">
          <div class="form-field">
            <label class="form-label">Temperature</label>
            <input
              v-model.number="directConfig.temperature"
              type="number"
              class="form-input"
              min="0"
              max="2"
          ...
          <div class="form-field">
            <label class="form-label">Max Tokens</label>
            <input
              v-model.number="directConfig.maxTokens"
              type="number"
              class="form-input"
              min="100"
              max="128000"
